...
```sql
-- Query to find the pickup zone with highest total revenue for Green taxis in 2020
SELECT 
  pickup_zone,
...
```sql
-- Query to find total monthly trips for Green taxis in October 2019
SELECT 
...
SELECT count(1) FROM taxi_rides_ny.prod.fhv_tripdata WHERE dispatching_base_num IS NOT NULL;
```
